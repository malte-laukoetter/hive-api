...
export namespace Bedrock {
  ...
  export class Methods {
    ...
    static PLAYER_MONTHLY = (game: GameType, player: Player) =>
      `${Methods.BASE_URL()}/game/monthly/player/${game}/${player}`;
  }

  export function monthlyLeaderboard(
    game: GameType.TREASURE_WARS
  ): Promise<TreasureWarsLeaderboardEntry[]>;
  export function monthlyLeaderboard(
    game: GameType.TREASURE_WARS,
    year: number,
    month: Month,
  ...
  export function monthlyLeaderboard(
    game: GameType.MURDER_MYSTERY
  ): Promise<MurderMysteryLeaderboardEntry[]>;
  export function monthlyLeaderboard(
    game: GameType.MURDER_MYSTERY,
    year: number,
    month: Month,
  ...
  export function monthlyLeaderboard(
    game: GameType.DEATH_RUN
  ): Promise<DeathRunLeaderboardEntry[]>;
  export function monthlyLeaderboard(
    game: GameType.DEATH_RUN,
    year: number,
    month: Month,
  ...
  export function monthlyLeaderboard(
    game: GameType.SURVIVAL_GAMES
  ): Promise<SurvivalGamesLeaderboardEntry[]>;
  export function monthlyLeaderboard(
    game: GameType.SURVIVAL_GAMES,
    year: number,
    month: Month,
  ...
  export async function monthlyLeaderboard(
    game: GameType,
    year: number = 60 * 60 * 1000,
    month: Month = null,
    maxCacheAge: number = 60 * 60 * 1000
  ): Promise<LeaderboardEntry[]> {
    return fetch(Methods.MONTHLY_LEADERBOARD(game, year, month), maxCacheAge);
  }

  export function playerMonthly(
    game: GameType.TREASURE_WARS,
    player: Player,
    maxCacheAge?: number
  ): Promise<TreasureWarsLeaderboardEntry>;
  ...
  export async function playerMonthly(
    game: GameType,
    player: Player,
    maxCacheAge: number = 60 * 60 * 1000
  ): Promise<LeaderboardEntry> {

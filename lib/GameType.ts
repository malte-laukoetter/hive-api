import {
  ...
  Game,
  Methods,
  gameInfoFactoryForGametype,
  playerGameInfoFactoryForGametype,
  AchievementInfo,
  AchievementInfoFactory,
  GameMap,
  GameTitle
} from "./main";
...
export class GameType {
  ...
  private info(maxCacheAge: number = 60 * 60 * 1000): Promise<GameTypeInfo> {
    return fetch(Methods.GAMETYPE_INFO(this.id), maxCacheAge).then(
      res =>
        new GameTypeInfo(
          res.uniqueplayers,
          res.achievements.map(a =>
            new AchievementInfoFactory().fromResponse(a).create()
          )
        )
    );
  }
   ...
   */
  latestGames = (maxCacheAge: number = 10 * 1000): Promise<Game[]> =>
    fetch(Methods.GAMETYPE_LATEST(this.id), maxCacheAge)
  ...
  maps(maxCacheAge: number = 24 * 60 * 60 * 1000): Promise<GameMap[]> {
    return fetch(Methods.MAP_LIST(this.id), maxCacheAge).then(res =>
  ...
  async titles(
    ...
    const titles = Object.values(res).map(
      (title: any) =>
        new GameTitle(
          this,
          title.name,
          title.required_points,
          title.human_name,
          title.plain_name,
          title.name_group ? title.name_group : undefined
        )
    );
    ...
    if (this.id === "BED") {
      titles.push(
        new GameTitle(this, "ZZZZZZ", -1, "&f&l✹Zzzzzz", "✹Zzzzzz", "Top Rank")
      );
    }
  ...
  specialLeaderboard(key): Leaderboard {
    if (!this.specialLeaderboardKeys().has(key))
      throw new Error(`Unknown special leaderboard ${key}`);
...
class GameTypeInfo {
  constructor(
    ...
    readonly achievements: [AchievementInfo]
  ) {}
}
...
export class BedGameType extends GameType {
  ...
  addSpecialLeaderboardKey(key: string): this {
    throw new Error(
      `Special Leaderboard Keys need to be added on the Base GameType`
    );
  }
...
export class GameTypes {
  ...
  static readonly BED = new GameType("BED", "BedWars")
    .addSubType(GameTypes.BEDS)
    .addSubType(GameTypes.BEDD)
    .addSubType(GameTypes.BEDT)
    .addSubType(GameTypes.BEDX)
    .addSpecialLeaderboardKey("win_streak");
   ...
   */
  static async update() {
    GameTypes._list = await fetch(Methods.GAMETYPE_LIST(), 0).then(res =>

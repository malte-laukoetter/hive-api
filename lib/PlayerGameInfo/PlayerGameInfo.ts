...
export class PlayerGameInfo {
  ...
  async currentTitle(
    ...
    if (Object.keys(this).indexOf("title") != -1) {
      const mayContainsTitle = titles.filter(
        title => title.name === (this as any).title
      );
      ...
      const mayContainsTitle = titles.filter(
        title => title.requiredPoints <= this.points && !title.topRank
      );
...
export abstract class PlayerGameInfoFactory<T>
  implements FromResponseFactory<T> {
...
export class RawPlayerGameInfoFactory extends PlayerGameInfoFactory<
  RawPlayerGameInfo
> {
  private _data;
...
export function createAchievementsFromAchievementResponse(
  type: GameType,
  data
): Achievement[] {

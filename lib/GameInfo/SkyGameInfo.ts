...
export class SkySoloGameInfoFactory extends SingleWinnerGameInfoFactory<
  SkySoloGameInfo
> {
  create(): SkySoloGameInfo {
    return new SkySoloGameInfo(
      this._gameEvents,
      this._server,
      this._endTime,
      this._startTime,
...
export class SkyTeamGameInfoFactory extends GameInfoFactory<SkyTeamGameInfo> {
  ...
  fromResponse(res: any): SkyTeamGameInfoFactory {
    return (super.fromResponse(res) as SkyTeamGameInfoFactory)
      ...
      .teamPlayers(
        Object.entries(res.teamplayers).map(
          ([color, players]: any) =>
            new SkyTeam(color, players.map(player => new Player(player)))
        )
      );
  }
...
export class SkyGameInfoFactory extends GameInfoFactory<GameInfo> {
  ...
  fromResponse(res: any): GameInfoFactory<GameInfo> {
    return (super.fromResponse(res) as SkyGameInfoFactory)
      .teams(res.teams)
      .fromResponse(res);
  }

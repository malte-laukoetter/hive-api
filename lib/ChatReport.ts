...
const MESSAGE_PARSE_REGEX =
  "[(?<day>dd)/(?<month>dd)/(?<year>dddd) (?<hour>dd):(?<minute>dd):(?<second>dd)]  (?<name>[a-zA-Z0-9_]+): (?<message>.*)";
...
export class ChatReportFactory implements FromResponseFactory<ChatReport> {
  ...
  fromResponse(res: any): ChatReportFactory {
    return this.player(new Player(res.UUID)).messages(res.messages);
  }

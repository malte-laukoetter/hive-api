...
export class Leaderboard {
  ...
  constructor(
    ...
    readonly specialLeaderboard: string = null
  ) {
  ...
  async load(
  ...
  ): Promise<Map<number, LeaderboardPlace>> {
    end--; // only the start positions are 0 indexed... what the hack is this even...
    if (
      start > Leaderboard.lastPlace ||
      end > Leaderboard.lastPlace ||
      start < Leaderboard.firstPlace ||
      end < Leaderboard.firstPlace
    )
    ...
    for (let range of Leaderboard.generateRequestRanges(requestPositions)) {
      ...
      if (this.specialLeaderboard) {
        requests.push(
          fetch(
            Methods.GAME_LEADERBOARD_SPECIAL(
              this.game.id,
              this.specialLeaderboard,
              range.start,
              range.end
            )
          )
        ...
        requests.push(
          fetch(Methods.GAME_LEADERBOARD(this.game.id, range.start, range.end))
        );
      }
    }
    ...
    results.forEach(res => {
      ...
      for (let place of res) {
        ...
        this.cache.set(
          place.index,
          new LeaderboardPlace(
            player,
            place.points || place.total_points || place.karma,
            place.index,
            place
          )
        );
  ...
  deleteCache() {
    Leaderboard.cache.set(
      `${this.game.id}${this.specialLeaderboard}`,
      new Map()
    );
  }
  ...
  async findPlayer(player: Player) {
    ...
    if (gameInfo.points < lastPlace.points) {
      ...
      let cachedPlace = Array.from(this.cache.entries()).filter(
        ([place, lP]) => lP.player.uuid === player.uuid
      );
      ...
      let place = Array.from((await this.load(min, max)).entries()).filter(
        ([place, lP]) => lP.player.uuid === player.uuid
      );

      if (place.length < 1)
        throw new Error("Player should be in the leaderboard but wasn't found");
  ...
  private async findPlayerByPrediction(
    player: Player,
    lastPlace: LeaderboardPlace
  ) {
    let predictedPlace = Leaderboard.predictedPlace(
      (await player.gameInfo(this.game)).points,
      lastPlace.points
    );
    ...
    let place = Array.from(places).filter(
      ([place, p]) => p.player.uuid === player.uuid
    );

...
export class ServerAchievement extends Achievement {
  info(maxCacheAge: number = 24 * 60 * 60 * 1000): any {
    return Server.achievements(maxCacheAge).then(
      list => list.filter(achievement => achievement.id == this.id)[0]
    );
  }
}
...
export class GameAchievement extends Achievement {
  ...
  async info(
    ...
    const possibleAchievementInfo = gameAchievements.filter(
      achievement => achievement.id == this.id
    );
...
export class TheSwarmAchievement extends ServerAchievement {
  constructor(
    ...
    progress: number,
    unlockedAt: Date,
    readonly theSwarmFrom: Player = null,
...
export class AchievementFactory implements FromResponseFactory<Achievement> {
    ...
    switch (this._type) {
      case AchievementTypes.SERVER:
        if (this._id == "THESWARM") {
          return new TheSwarmAchievement(
            this._id,
            this._progress,
            this._unlockedAt,
            this._theSwarmFrom,
  ...
  fromResponse(res: any): FromResponseFactory<Achievement> {
    this.progress(res.progress).unlockedAt(
      createDateFromResponse(res.unlockedAt)
    );
...
export function theSwarmGameToGameType(name: string): GameType {
  switch (name.toLowerCase()) {
    ...
    default:
      ...
      let matchingTypes = GameTypes.list.filter(
        type =>
          type.id === name.toUpperCase() ||

...
export class Server {
  ...
  static achievements(
    ...
    return fetch(Methods.GLOBAL_ACHIEVEMENT_LIST(), maxCacheAge).then(res =>
      res.map(achievement =>
        new AchievementInfoFactory().fromResponse(achievement).create()
      )
    );
  }
  ...
  static uniquePlayers(maxCacheAge: number = 60 * 60 * 1000): Promise<Number> {
    return fetch(Methods.UNIQUE_PLAYER_COUNT(), maxCacheAge).then(
      res => res.count
    );
  }
